...
export function BarraNavegacion2({ routes, brandImg, brandName }) {
  ...
  return (
    <aside
      ...
      } fixed inset-0 z-50 my-4 ml-4 h-[calc(100vh-32px)] w-72 transition-transform duration-300 xl:translate-x-0   rounded-2xl shadow-2xl shadow-purple-800 `}
    >
      <div className={`relative `}>
        <Card
          className={`w-auto shadow-none mb-0 ${
            sidenavType === "dark" ? "bg-pink-500" : "bg-pink-100"
          }`}
        >
          <CardHeader
            floated={false}
